# Split-PDF: Complete Raw Content
...
- If the file is NOT already inside `./output/articles/`, copy it there (do not move — preserve the original location)
- Proceed to Step 2
...
3. Save it to `./output/articles/` in the project directory (create the directory if needed)
4. Proceed to Step 2

**CRITICAL: Always preserve the original PDF.** "The downloaded or provided PDF in `./output/articles/` must NEVER be deleted, moved, or overwritten at any point" in the workflow. The split files are derivatives — the original is the permanent artifact. Do not clean up, do not remove, do not tidy. The original stays.
...
The original PDF remains in `output/articles/` permanently. The splits are working copies. If anything goes wrong, you can always re-split from the original.
...
```
output/articles/split_smith_2024/notes.md
```
